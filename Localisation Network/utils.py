...
def angle_error_regression(y_true, y_pred):
    return K.mean(angle_difference(y_true * MAX_ANGLE, y_pred * MAX_ANGLE))
...
class RotNetDataGenerator(Iterator):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        for i, j in enumerate(index_array):
            ...
            if self.rotate:
                # get a random angle
                rotation_angle = (np.random.randint(2*MAX_ANGLE) - MAX_ANGLE) % 360
            else:
                rotation_angle = 0
...
def get_deformations(model, input, num_images=5, size=None, crop_center=False,
                     crop_largest_rect=False, preprocess_func=None, save_path=None,
                     display=False, max_angle=180):
    if isinstance(input, (np.ndarray)):
        images = input
    ...
    return y_pred, x_rot.reshape(x_rot.shape[0], x_rot.shape[1], x_rot.shape[2])  # return predictions, rotated images
