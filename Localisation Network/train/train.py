...
# data = np.load('train.p', allow_pickle=True)
#
# input_dim = 32
#
# temp = np.zeros((data['features'].shape[0], input_dim, input_dim))
# for idx in range(data['features'].shape[0]):
#     temp[idx] = cv2.cvtColor(data['features'][idx], cv2.COLOR_BGR2GRAY)
# X_train = temp.copy()
#
# data = np.load('test.p', allow_pickle=True)
#
# temp = np.zeros((data['features'].shape[0], input_dim, input_dim))
# for idx in range(data['features'].shape[0]):
#     temp[idx] = cv2.cvtColor(data['features'][idx], cv2.COLOR_BGR2GRAY)
